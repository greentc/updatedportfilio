    ...
    def get(self):
        try:
            ...
            if self.request.path == '/projects.html':
                self.response.write(template.render({'Home' : 'Home', 'Projects' : 'PROJECTS', 'Contact' : 'Contact', 'Journey': 'Journey' ,'activep' : 'factive'} ))
                logging.info("Projects template rendered")

            if self.request.path =='/contact.html':
                self.response.write(template.render({'Home' : 'Home', 'Projects' : 'Projects', 'Contact' : 'CONTACT', 'Journey': 'Journey' ,'activec' : 'active'} ))
                logging.info("Contact template rendered)")

            if self.request.path == '/journey.html':
                self.response.write(template.render({'Home' : 'Home', 'Projects' : 'Projects', 'Contact' : 'Contact', 'Journey': 'JOURNEY', 'activej' : 'active'} ))
                logging.info("Journey template rendered")

            if self.request.path == '/home.html':
                self.response.write(template.render({'Home' : 'HOME', 'Projects' : 'Projects', 'Contact' : 'Contact', 'Journey': 'Journey', 'activeh' : 'active' } ))
                logging.info("Home template rendered")
        except:
            template = JINJA_ENVIRONMENT.get_template('templates/home.html')
            self.response.write(template.render({'Home': 'HOME', 'Projects' : 'Projects', 'Contact' : 'Contact', 'Journey': 'Journey'} ))
